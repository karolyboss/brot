// BrainRot Presale Website JavaScript - Ultra Debug Version
...
class BrainRotPresale {
    ...
    setupElements() {
        ...
        console.log('📋 Element status:', {
            ...
            purchaseModal: !!this.purchaseModal,
            solInput: !!this.solInput,
            confirmBtn: !!document.getElementById('confirm-purchase')
        });
    }
    ...
    setupEventListeners() {
        ...
        // CONFIRM PURCHASE BUTTON - CRITICAL
        const confirmPurchaseBtn = document.getElementById('confirm-purchase');
        if (confirmPurchaseBtn) {
            console.log('✅ CONFIRM PURCHASE BUTTON FOUND - Adding listener');
    ...
    async createPurchaseTransaction(solAmount) {
        try {
            ...
            // Test wallet responsiveness - More permissive approach
            try {
                console.log('🔍 Testing wallet responsiveness...');
                console.log('Wallet object type:', this.wallet?.constructor?.name);
                ...
                let balanceInfo = 'unknown';

                try {
                    if (typeof this.wallet.getBalance === 'function') {
                        const testBalance = await this.wallet.getBalance(this.publicKey);
                        balanceInfo = `${testBalance / 1000000000} SOL`;
                        walletResponsive = true;
                    } else if (this.wallet.request && typeof this.wallet.request === 'function') {
                        const testBalance = await this.wallet.request({
                            method: 'getBalance',
                            params: [this.publicKey.toString()]
                        });
                        balanceInfo = `${testBalance / 1000000000} SOL`;
                        walletResponsive = true;
                    } else if (window.solana && window.solana.getBalance) {
                        const testBalance = await window.solana.getBalance(this.publicKey);
                        balanceInfo = `${testBalance / 1000000000} SOL`;
                        walletResponsive = true;
                    }
                } catch (balanceTestError) {
                    console.log('⚠️ Balance test failed, but continuing:', balanceTestError.message);
                    balanceInfo = 'check failed';
                    walletResponsive = true; // Assume responsive if we can't test
                }

                console.log(`✅ Wallet status: ${walletResponsive ? 'responsive' : 'unresponsive'}, balance: ${balanceInfo}`);

                if (!walletResponsive) {
                    throw new Error('Wallet not responsive');
                }

            } catch (testError) {
                console.error('❌ Wallet responsiveness test failed:', testError);
                // Don't fail here - some wallets might not support balance checking
                // Let's try to proceed with the transaction anyway
                console.log('⚠️ Proceeding with transaction despite responsiveness test failure');
            }

            // Check balance with fallback methods - More permissive
            try {
                let balance;
                let balanceInSOL = 0;
                ...
                try {
                    if (typeof this.wallet.getBalance === 'function') {
                        ...
                        balance = await this.wallet.request({
                            method: 'getBalance',
                            params: [this.publicKey.toString()]
                        });
                    } else if (window.solana && window.solana.getBalance) {
                        balance = await window.solana.getBalance(this.publicKey);
                    } else {
                        throw new Error('No balance method available');
                    }

                    balanceInSOL = balance / 1000000000;
                    ...
                    console.log('⚠️ Balance check failed, proceeding anyway:', balanceCheckError.message);
                    // Don't fail the transaction just because we can't check balance
                    // Some wallets might not support balance checking
                }

            } catch (balanceError) {
                console.error('❌ Balance check error:', balanceError);
                // Don't fail here - proceed with transaction
                console.log('⚠️ Proceeding with transaction despite balance check failure');
            }
            ...
            try {
                ...
                if (typeof this.wallet.signAndSendTransaction === 'function') {
                    ...
                    console.log('✍️ CREATING TRANSACTION - This should trigger Phantom popup...');
                    console.log('🔐 About to call wallet.signAndSendTransaction()...');
                    ...
                    console.log('⚠️ signAndSendTransaction not available, wallet type:', this.wallet.constructor.name);
